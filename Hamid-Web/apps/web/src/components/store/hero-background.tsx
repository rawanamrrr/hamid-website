...
import type { StoreHeroImageView } from "@/lib/store/queries";

const SLIDE_INTERVAL_MS = 6000;

/** Intro banner background — crossfades through the admin-managed hero images. */
export function StoreHeroBackground({ images }: { images: StoreHeroImageView[] }) {
  const [active, setActive] = useState(0);

  useEffect(() => {
    if (images.length < 2) return;
    const timer = setInterval(() => setActive((i) => (i + 1) % images.length), SLIDE_INTERVAL_MS);
